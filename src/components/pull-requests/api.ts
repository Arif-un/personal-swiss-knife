...
export const prApi = {
  ...
  checks: (repo: string, number: number) =>
    invoke<PrCheck[]>("fetch_pr_checks", { repo, number }),
};
...
export const prKeys = {
  list: (repo: string, filters: Filters) =>
    ["pull-requests", repo, filters] as const,
  ciCounts: (repo: string, numbers: number[]) =>
    ["ci-label-counts", repo, numbers] as const,
  ...
  mergeQueue: (repo: string, numbers: number[]) =>
    ["merge-queue-status", repo, numbers] as const,
  checks: (repo: string, number: number) =>
    ["pr-checks", repo, number] as const,
